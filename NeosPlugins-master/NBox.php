 ...
 * @version 0.1
 * @api 4.0.0
 * @description Neos is zonzal, injung kak
 */
...
class NBox extends PluginBase implements Listener {
	...
	public function onEnable () {
		...
		$this->idbase = new Config ($this->getDataFolder().'id.yml',Config::YAML);
		$this->id = $this->idbase->getAll();
	...
	{ 
        ...
        $command->setDescription($name.' 명령어 | 플러그인 구매 문의 카카오톡 n4kr');
	...
	public function onDisable () {
		...
		$this->database->save();
		$this->idbase->setAll ($this->id);
		$this->idbase->save();
	}
	
	public function go ($player, $box) {
		$db = $this->db [$box];
		$rand = mt_rand (1, 100);
		if ($rand < $db ['failure']) {
			$this->msg ($player, '아쉽지만 꽝입니다 ㅜㅠㅠㅜ 다음 기회를 노려보세요!');
		} else {
			for ($i = 0; $i < 9999999; $i++) {
				if (!isset($db['rewards'][$i])) {
					break;
				}
			}
			$number = mt_rand (0, $i-1);
			$item = Item::jsonDeserialize($db['rewards'][$number]);
			$player->getInventory()->addItem ($item);
			$this->msg ($player, $this->getItemName ($item).'(이)가 당첨되었습니다! 축하합니당');
		}
	}
	...
	public function onTouch (PlayerInteractEvent $event) {
		...
		$bode = $event->getBlock()->getId().':'.$event->getBlock()->getDamage();
		if ($bode == $this->setting ['portal-block']) {
			if (isset($this->id[$code])) {
				if ($this->setting ['touch'] == 'true') {
					$a = explode (':', $code);
					$player->getInventory()->removeItem (Item::get($a[0],$a[1],1));
					$this->go ($player, $this->id[$code]);
				} else {
					$this->msg ($player, '터치를 통한 뽑기는 금지되어 있습니다');
				}
			} else {
				$this->msg ($player, '뽑기권을 들고 포탈을 터치해주세요');
			}
		}
	}
	...
	public function sendUI ($player) {
		$arr = [];
		array_push($arr,array('text' => '- 창 종료하기'));
        foreach ((array)$this->db as $name => $null){
            array_push($arr,array('text' => '- '.$name));
        }
        $text = [
            'type' => 'form',
            'title' => '[ N | 뽑기 시스템 ]',
            'content' => '뽑고 싶은 박스를 선택해주세요',
            "buttons" => $arr
        ];
		
		$packet = new ModalFormRequestPacket();
		$packet->formId = 123456789;
		$packet->formData = json_encode ($text);
        ...
        if ($packet instanceof ModalFormResponsePacket) {
			...
			if ($id == 123456789) {
				if (!$val == null) {
					...
					$a = explode (':', $this->db[$arr[$val]] ['item']);
					if ($player->getInventory()->contains (Item::get($a[0], $a[1], 1))){
						$player->getInventory()->removeItem (Item::get($a[0],$a[1],1));
						$this->go ($player, $arr[$val]);				
					} else {
						$this->msg ($player, '뽑기권이 부족합니다');
					}
				}
			}
		}
	...
	public function onCommand (CommandSender $player, Command $command, string $label, array $args):bool {
		...
		if ($command->getName() == '뽑기') {
			if ($this->setting ['command'] == 'true') {
				$this->sendUI($player);
			} else {
				$this->msg ($player, '명령어를 통한 뽑기는 금지되어 있습니다');
			}
		}
		if ($command->getName() == '뽑기생성' && $player->isOp()) {
			if (isset($args[1])) {
				$this->db [$args[0]] = [];
				$this->db [$args[0]] ['item'] = $item->getId().':'.$item->getDamage();
				$this->db [$args[0]] ['rewards'] = [];
				$this->db [$args[0]] ['failure'] = $args[1];
				$this->id [$this->db[$args[0]]['item']] = $args[0];
				$this->msg ($player, $args[0].'가 생성되었습니다 (뽑기권 - '.$this->getItemName ($item).', 실패 확룰 - '.$args[1].')');
				$this->msg ($player, $args[0].'에 아이템을 추가하고 싶다면 "/뽑기추가 '.$args[0].'"를 입력하시면 됩니다');
			} else {
				$this->msg ($player, '뽑기생성 [박스 이름] [실패 확률] | 뽑기권은 손에 들고 있는 아이템으로 지정됩니다');
			}
		} else if ($command->getName() == '뽑기추가' && $player->isOp()) {
			if (isset($args[1])) {
				if (isset($this->db[$args[0]])) {
					$array = ['id' => $item->getId(), 'damage' => $item->getDamage(), 'count' => $args[1], 'nbt' => $item->getCompoundTag()];
					for ($i = 0; $i < 9999999; $i++) {
						if (!isset($this->db[$args[0]]['rewards'][$i])) {
							break;
						}
					}
					$this->db [$args[0]]['rewards'][$i] = $array;
					$this->msg ($player, $args[0].'에 보상을 추가했습니다 (아이템 번호 - '.$i.', 아이템 코드 - '.$item->getId().', 아이템 데미지 - '.$item->getDamage().', 아이템 개수 - '.$args[1].')');
					$this->msg ($player, '방금 추가된 아이템을 삭제하고 싶다면 "/뽑기제거 '.$args[0].' '.$i.'"를 입력하시면 됩니다');
				} else {
					$this->msg ($player, $args[0].'(이)라는 박스가 없습니다! 이름을 확인해주세요');
				}
			} else {
				$this->msg ($player, '뽑기추가 [박스 이름] [아이템 개수] | 추가될 아이템은 손에 들고 있는 아이템으로 지정됩니다 (아이템의 NBT도 저장됩니다)');
			}
		} else if ($command->getName() == '뽑기제거') {
			if (isset($args[1])) {
				if (isset($this->db[$args[0]])) {
					if (isset($this->db[$args[0]]['reward'][$args[1]])) {
						$this->msg ($player, $args[0].'에서 '.$args[1].'번 아이템을 제거했습니다');
						unset ($this->db[$args[0]]['reward'][$args[1]]);
					} else {
						$this->msg ($player, $args[0].'에서 '.$args[1].'번 아이템이 존재하지 않습니다');
					}
				} else {
					$this->msg ($player, $args[0].'(이)라는 박스가 없습니다! 이름을 확인해주세요');
				}
			} else {
				$this->msg ($player, '뽑기제거 [박스 이름] [아이템 번호] | 박스에 추가된 아이템을 제거합니다');
			}
		} else if ($command->getName() == '뽑기삭제') {
			if (isset($args[0])) {
				if (isset($this->db[$args[0]])) {
					$this->msg ($player, $args[0].'(을)를 제거하였습니다');
					unset ($this->id [$this->db[$args[0]]['item']]);
					unset ($this->db[$args[0]]);
				} else {
					$this->msg ($player, $args[0].'(이)라는 박스가 없습니다! 이름을 확인해주세요');
				}
			} else {
				$this->msg ($player, '뽑기삭제 | 박스를 삭제합니다');
			}
		}
		return true;
	}

 	public function msg ($player, $m) {
 		$player->sendMessage ('§b§l[§fNBox§b] §r'.$m);
 	}
